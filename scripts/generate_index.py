    ...
    for root, _, files in os.walk(CONTENT_DIR):
        for file in files:
            if file == "index.md":
                ...
                if p:
                    all_posts_pt.append(p)
            elif file == "index.en.md":
                p = parse_post(os.path.join(root, file), lang="en")
                if p:
                    all_posts_en.append(p)

    # Filtros por data (ignora futuros se não houver flag)
    if not include_future:
        all_posts_pt = [
            p for p in all_posts_pt if p["date"].timestamp() <= now.timestamp()
        ]
        all_posts_en = [
            p for p in all_posts_en if p["date"].timestamp() <= now.timestamp()
        ]

    # Separação por categorias (Tags)
    def is_off_topic(p):
        return "off-topic" in p["tags"]
