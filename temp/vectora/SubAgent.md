...
> [!IMPORTANT] > **Decisão Arquitetural Estratégica** | Esta não é uma limitação técnica — é uma escolha deliberada para
> garantir controle, validação e qualidade operacional.
...
> 💡 **Insight**: O Vectora não é apenas um "provedor de contexto". É um **interpretador especializado** que traduz
> queries genéricas em operações vetoriais precisas — algo que agents principais não fazem e não querem fazer.
...
Mesmo para tools que **não** envolvem embedding (ex: `file_read`, `grep_search`), atuar como MCP tools puro significa
perder:
...
> [!TIP] > **Você pode.** Mas não vai ter o que torna Vectora único.
...
> 💡 **Recomendação**: Comece com o Sub-Agent completo. Se depois de validar o valor você quiser simplificar, a opção
> `--mode tools-only` está disponível. Mas a maioria dos usuários descobre que as camadas de controle são justamente o
> que faltava.
...
> _"Vectora não é mais um conjunto de tools MCP. É a camada que faz qualquer agent funcionar melhor em código — com
> controle, validação e segurança por design."_
