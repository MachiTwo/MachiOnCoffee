...
> [!IMPORTANT] > **Regra Imutável**: `node_modules`, `.git` e outros paths hard-coded **nunca** são processados —
> independente de `.embedignore`, configurações ou prompts.

Sim, lembro perfeitamente. Essa é uma das decisões mais importantes da arquitetura de segurança e performance do
Vectora.
...
export class Guardian {
  static isHardBlocked(filePath: string): boolean {
    ...
    if (HARD_EXCLUDE_DIRS.some((dir) => filePath.split(path.sep).includes(dir))) return true;

    // 2. Verifica extensões hard-coded
    if (HARD_EXCLUDE_EXTENSIONS.includes(ext)) return true;

    // 3. Verifica patterns hard-coded
    if (HARD_EXCLUDE_PATTERNS.some((pattern) => pattern.test(filePath))) return true;
...
export async function parseStandardIgnores(root: string): Promise<string[]> {
  ...
  const files = [".gitignore", ".npmignore", ".dockerignore", ".prettierignore", ".eslintignore"];
...
> [!TIP] > `.embedignore` só afeta o pipeline de embedding/RAG.  
> Arquivos ignorados aqui ainda podem ser lidos pelo agent via `file_read` se estiverem no Trust Folder e não forem
> hard-blocked.
...
export async function shouldProcessFile(filePath: string, root: string): Promise<boolean> {
